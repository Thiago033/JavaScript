...
const versao = new Discord.MessageEmbed()
  .setColor('0099ff')
  .setAuthor('Bot ta vivo!')
  .setTitle('Bot de musica muito legal')
...
const comandos = new Discord.MessageEmbed()
  ...
  .setDescription("Se digitar alguma coisa errada e bugar tudo a culpa nao eh minha KEKW")
  .addField("----------------------------------", "👇")
...
client.on("message", async message => {
  ...
  if (message.channel.id !== canalComandos) {
      message.channel.send("Voce nao pode digitar comando nesse canal, usa o canal certo bobao!");
      return;
  }
  ...
  if (message.content.startsWith(`${prefix}play`)) {
      ...
      message.channel.send("Digitou o comando errado bobao!");
  }
});
...
async function execute(message, serverQueue) {
  ...
  if(!validate) return message.channel.send('Digita o link certo ai bobao!');
  //---------------------------------------

  const voiceChannel = message.member.voice.channel;
  if (!voiceChannel)
    return message.channel.send(
      "Precisa entrar em um canal de voz pra tocar a musica bobao!"
    );
